记录实验设置：
    ...
    validate数据集前1000场景训练，sample数据集测试：
        ...
        纯SAC：2023-03-15_16-46

        2023-03-16_09-42：将奖励中碰撞车距的阈值由2改1，有效解决了之前IL loss下降和RL奖励上升存在冲突的问题！预训练，从100000开始训练。
        2023-03-16_13-22：将奖励中碰撞车距的阈值由2改1，从零开始训练。
        2023-03-16_17-18：将奖励中碰撞车距的阈值由2改1，从100000开始训练，但调小了学习率。奖励有在上升，loss有在下降，但是就是车道保持、转弯和红绿灯的停车、启动变差，可能本身还是得把原本的预训练模型加上这些输入才能加强化学习？只靠模仿来实现这些很容易就被强化学习破坏掉？
        2023-03-17_10-57：在上一个的基础上，加入了模仿奖励，希望它能首先尽可能保持模仿的效果。从loss曲线初步来看，波动减小，但是闭环测试中一开始还是破坏了红绿灯和正确车道保持的效果。
        2023-03-17_18-02：在上一个的基础上，通过给policy loss乘以0.01系数来调小rl loss占比，希望它起到辅助作用而不是破坏已有模型。
        (此外换用1env会不会有效加速训练？)
        经过验证。。在100000步效果好的预测模型基础上同样地再训练下去，仍然会发生破坏红绿灯和车道保持的现象。。说明本身100000模型就是一个好的特例而已？！

        2023-03-18_20-42: 将奖励中碰撞车距的阈值由2改1，从100000开始训练，但调小了学习率到3e-5，通过给policy loss乘以0.01系数来调小rl loss占比。
        2023-03-19_10-22: 将奖励中碰撞车距的阈值由2改1，从100000开始训练，但调小了学习率到3e-5，通过给policy loss乘以0.1系数来调小rl loss占比，在上面的基础上把训练间隔由100epoch改为50epoch。RL 和 IL 的loss都在下降但是波动大，其实训练没啥问题，就是这个波动导致的闭环测试中转弯、红绿灯问题较大，为此我下面的工作应该是：1.完成直接的闭环测试和指标输出代码；2.通过添加路由输入（和信号灯指示）来解决波动大的问题。

        从闭环指标结果来看，好像2023-03-17_18-02是非常有前景的一种解决方案？着重解决它的问题or在其基础上探索新的方案
        2023-03-21_09-07: 添加0.1倍模仿学习奖励，将奖励中碰撞车距的阈值改1.5，从80000开始训练，学习率3e-5，0.01倍policy loss系数, 采样间隔为25epoch，10个并行采样env，episode步长128。

        2023-03-23_11-05: 添加了转弯路由信息channel到图像中，重新进行预测训练，期望能降低IL loss波动。然而从轨迹预测的训练方式上看，转弯信息其实并不是造成其波动大的主要原因？存在models_pretrain_withturn
        2023-03-23_12-11: 添加了转弯路由信息channel到图像中，重新进行预测训练，并且调大il batch size 64，存在models_pretrain_withturn1  效果非常好。batch size适当增大帮助学习信号灯；转向信息channel帮助学习转弯；都有助于减小波动、提升学习性能？！现有问题是，加的转向信号并不能在闭环测试作用好，可能需要适当增加其信号覆盖的时间范围，只要某时刻存在转弯则之前之后都有相应的标志；或者加入历史轨迹信息结合未来轨迹信息作判断；或者直接升级，根据车头朝向将转向信号变为指向信号？根据未来x y 坐标指向将行进方向八分，感觉这个靠谱！！
        2023-03-23_22-12: 基于64 batch size的上述预测训练方案的PPO强化学习（简单车距奖励版），奖励上升困难、波动大，原有效果被破坏出现不按路线行驶情况。
        2023-03-24_15-22: 基于64 batch size的上述预测训练方案的PPO强化学习（简单车距奖励+基于dist_ref_traj的off-road奖励版，并且off-road时终止episode）有一定作用但波动还是大；
        2023-03-24_16-05: 在上面基础上将0.01 policy loss改为 0.01 RL loss，并且不再off-road时终止；
        2023-03-24_17-04: 上述方法不行，改成大惩罚截断型RL训练，基于64 batch size的上述预测训练方案的PPO强化学习（基于collision的+基于dist_ref_traj的off-road奖励版，并且碰撞或off-road时终止episode）；结果基本能保留原本的效果不变差太多，但是奖励还是上升困难，碰撞提升有限。
        2023-03-25_11-07: 改成0.1 policy loss试试；奖励设计的不好，抖震变大。
        2023-03-25_23-16: 恢复0.01 policy loss 并去掉off-road奖励试试，只基于collision惩罚并终止episode，非碰撞时有0.1奖励。
        2023-03-26_17-09: 先在上行基础上调gae lambda 到0.5以缩小RL Value方差试试，不行; 并缩小RL epoch训练间隔到10试试；调整gamma折扣因子到0.995试试,不行；调小学习率到3e-6试试，有一定效果；tb_log_2023-03-26_18-37调小clip start 到0.05,有一定效果；tb_log_2023-03-26_19-07调小value loss 到0.005,有效减小了IL 部分的抖震波动；tb_log_2023-03-26_20-35在基础上改为256-rollout_step, 64-eps_length, 128-batch_size，效果不大; tb_log_2023-03-26_21-58调小clip start 到0.01, end 到0.1不太好用；调大entropy 系数至0.05；

        tb_log_2023-03-27_10-01: 预训练模型的基础上只在测试100场景中发生碰撞的几个bad case场景中训练试试；添加clip_range_vf=0.2, 没啥用；先只在1个场景调好效果，

        先把PPO效果调好，并且让奖励函数不与模仿Loss冲突；在sample训练记得改rescale参数。