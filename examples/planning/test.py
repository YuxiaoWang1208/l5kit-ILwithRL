...
if __name__ == "__main__":
    ...
    # define model
    model_path = "./models/planning_model" + str(steps) + ".pt"
    ...
    def rollout_episode(model, env, idx = 39):  # 18 25 29 50 60 65 75 95
        ...
        while True:
            ...
            image = th.from_numpy(obs["image"])
            il_action = th.from_numpy(action)
            action = model.predict(image).squeeze(0)
            # print(env.get_attr('non_kin_rescale'))
            rl_action = action
            criterion = th.nn.MSELoss(reduction="none")
            il_loss = th.mean(criterion(il_action, rl_action))
            print(il_loss)
            action = action[0].unsqueeze(0)  # 12 future steps
            action = action.numpy()  
